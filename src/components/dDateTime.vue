        ...
        <div class="calendar-move" v-if="isShow">
          <div class="d-date-time-contain" @click.stop v-if="isShow">
            <div class="d-date-time-header" :style="{background: color}">
              <div class="d-date-time-year" v-if="type === 'dateTime' || type === 'year' || type ==='date'"
    ...
    methods: {
      ...
      addMaxDisable (arr, type, max) {
        ...
        if (type === 'month') {
          ...
          maxDate = arr.findIndex((item) => {
            return item.isMonth && new Date(this.max).getTime() < new Date(this.date.getFullYear(), this.date.getMonth(), item.value).getTime()
          })
          maxDate--
        }
        if (maxDate <= -1) {
